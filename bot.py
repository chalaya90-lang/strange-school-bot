...
EMOJI_RIDDLES = [
    {"emoji": "🦁👑", "answers": ["король лев", "король-лев"], "hint": "мультфільм Disney"},
    {"emoji": "🧊❄️👸", "answers": ["крижане серце", "холодне серце", "frozen"], "hint": "мультфільм про Ельзу"},
    {"emoji": "🕷️👨", "answers": ["людина павук", "людина-павук", "spiderman", "spider man"], "hint": "супергерой Marvel"},
    {"emoji": "🦸🦸‍♀️🦸‍♂️", "answers": ["суперсімейка"], "hint": "мультфільм Pixar"},
    {"emoji": "🐟🐠🔵", "answers": ["немо", "в пошуках немо"], "hint": "мультфільм про рибку"},
    {"emoji": "🤖🚗", "answers": ["трансформери", "трансформер"], "hint": "роботи що перетворюються"},
    {"emoji": "🧙‍♂️⚡️🏫", "answers": ["гаррі поттер", "гарі поттер", "harry potter"], "hint": "школа чарівників"},
    {"emoji": "🦖🏝️", "answers": ["парк юрського періоду", "парк юрського периоду", "юрський парк"], "hint": "динозаври на острові"},
    {"emoji": "👸🐸", "answers": ["принцеса і жаба", "принцеса та жаба"], "hint": "мультфільм Disney"},
    {"emoji": "🐻🍯", "answers": ["вінні пух", "вінні-пух", "вінні"], "hint": "ведмедик любить мед"},
    {"emoji": "🚀👨‍🚀🌕", "answers": ["аполлон", "місія аполлон"], "hint": "місія на Місяць"},
    {"emoji": "🧸❤️🤖", "answers": ["валл і", "валл-і", "wall-e", "walle"], "hint": "робот у космосі"},
    {"emoji": "🦊🐰🏙️", "answers": ["зоотрополіс", "зоотопія", "zootopia"], "hint": "місто тварин"},
    {"emoji": "🎪🐘🤡", "answers": ["дамбо", "dumbo"], "hint": "літаючий слоник"},
    {"emoji": "🌊🏄‍♀️🌺", "answers": ["моана", "moana"], "hint": "дівчина і океан"},
    {"emoji": "🐉🔥🧒", "answers": ["як приручити дракона", "приручити дракона", "дракон", "беззубик"], "hint": "хлопець і дракон Беззубик"},
    {"emoji": "🍕🗽🐢", "answers": ["черепашки ніндзя", "черепашки-ніндзя", "tmnt"], "hint": "герої в панцирі"},
    {"emoji": "⚡️🏎️🏁", "answers": ["блискавка маккуїн", "блискавка маккуін", "маккуїн", "маккуін", "тачки", "cars"], "hint": "мультфільм Тачки"},
    {"emoji": "🦁🐯🐻", "answers": ["книга джунглів", "маугли", "мауглі"], "hint": "Мауглі і тварини"},
    {"emoji": "👦🎈🔴", "answers": ["воно", "it"], "hint": "страшний клоун"},
    {"emoji": "🦸‍♀️💪🌟", "answers": ["диво жінка", "диво-жінка", "wonder woman"], "hint": "супергероїня DC"},
    {"emoji": "🐼🥋🍜", "answers": ["kung fu panda", "кунг фу панда", "кунг-фу панда", "панда"], "hint": "панда і кунг-фу"},
    {"emoji": "🏰👸🐲", "answers": ["відважна", "brave", "мерида"], "hint": "шотландська принцеса Мерида"},
    {"emoji": "🤥👃🪵", "answers": ["піноккіо", "pinocchio"], "hint": "дерев'яний хлопчик"},
    {"emoji": "🌹👹", "answers": ["красуня і чудовисько", "красуня та чудовисько", "beauty and the beast"], "hint": "казка Disney"},
    {"emoji": "🧞‍♂️🪔✨", "answers": ["аладдін", "аладін", "aladdin"], "hint": "джин і три бажання"},
    {"emoji": "🐠🦈🌊", "answers": ["в пошуках немо", "пошуки немо", "немо"], "hint": "тато шукає сина"},
    {"emoji": "🎭😂😢", "answers": ["головоломка", "inside out"], "hint": "емоції всередині голови"},
]
...
holidays_sheet   = None
baskets_sheet    = None
vacations_sheet  = None

def init_google():
    global gc, users_sheet, bank_sheet, ideas_sheet, schedule_sheet
    global absence_sheet, purchases_sheet, complaints_sheet
    global holidays_sheet, baskets_sheet, vacations_sheet
    try:
...
# ================= БОТ (оголошення ПЕРЕД хендлерами) =================
...
weekly_purchases: dict[str, dict] = {}
...
def week_str() -> str:
    return datetime.now(kyiv).strftime("%Y-W%W")
...
# Одна правильна версія teacher_kb як змінна (не функція)
...
async def cmd_award(msg: Message):
    uid = str(msg.chat.id)
    if not is_admin(uid):
        return
...
async def cmd_awardid(msg: Message):
    uid = str(msg.chat.id)
    if not is_admin(uid):
        return
    parts = (msg.text or "").split(maxsplit=3)
...
async def cmd_fineid(msg: Message):
    uid = str(msg.chat.id)
    if not is_admin(uid):
        return
    parts = (msg.text or "").split(maxsplit=3)
...
async def cmd_classfire(msg: Message):
    uid = str(msg.chat.id)
    if not is_admin(uid):
        return
    parts = (msg.text or "").split(maxsplit=2)
...
async def cmd_classbonus(msg: Message):
    uid = str(msg.chat.id)
    if not is_admin(uid):
        return
    parts = (msg.text or "").split(maxsplit=2)
...
async def cmd_bugfix(msg: Message):
    uid = str(msg.chat.id)
    if not is_admin(uid):
        return
    egor_uid    = "5466651089"
    ...
    bug_text = (
        ...
        "Хлопці — красавці! За кмітливість і винахідливість кожен отримує +10 🪙\n\n"
        "Але баг вже виправлено 😄 Так що тепер по чесному!"
    )
    users = get_all_users()
    for tuid in users:
        try:
...
async def handler(msg: Message):
    ...
    if user_states.get(uid) == "challenge_report":
        ...
        for aid, data in pending_approvals.items():
            if data["uid"] == uid and data["action"] == "challenge":
                ...
                challenge_aid = aid
                break
        approve_line = f"✅ /approve {challenge_aid}" if challenge_aid else "(заявка не знайдена)"
        await notify_admin(
            f"🎯 Звіт про челендж!\n\n"
            f"Учень: {name}\n"
    ...
    if user_states.get(uid, "").startswith("basket_contribute:"):
        ...
        if remaining <= 0:
            ...
            await msg.answer(
                f"✅ Ти вніс {amount} 🪙\nЗібрано: {new_collected}/{basket['price']} 🪙\nЩе треба: {remaining} 🪙",
    ...
    if user_states.get(uid) in ("teacher_plus", "teacher_minus"):
        ...
        if user_states[uid] == "teacher_plus":
            ...
            try:
                await bot.send_message(int(target_uid), f"🎉 Вчитель нарахував тобі +{amount} 🪙!")
            except Exception:
                pass
        else:
            remove_coins_from(target_uid, amount)
            await msg.answer(f"✅ {target_name} -{amount} 🪙", reply_markup=teacher_kb)
            try:
                await bot.send_message(int(target_uid), f"⚠️ Вчитель зняв {amount} 🪙")
            except Exception:
                pass
        user_states.pop(uid, None)
        return
    ...
    if user_states.get(uid) == "teacher_fine":
        ...
        for tuid in get_all_users():
            try:
                await bot.send_message(int(tuid), f"⚠️ Вчитель зняв {amount} 🪙 з банку класу!")
            except Exception:
                pass
        return
    ...
    if text == "📅 Розклад":
        ...
        await msg.answer(format_schedule(lessons, DAYS_UA[weekday]))
        return
    ...
    if text == "💡 Ідеї для класу":
        if check_daily(uid, "idea"):
            await msg.answer("💡 Сьогодні ідею вже подав! Повертайся завтра 😊")
            return
    ...
    if text == "💌 Написати добро":
        if not is_active("добро"):
            ...
            return
        if check_daily(uid, "good"):
            await msg.answer("💌 Добро сьогодні вже написав! Повертайся завтра 😊")
            return
    ...
    if text == "🎰 Удача":
        ...
        plays_this_week = [d for d in plays if d == wk]
        if len(plays_this_week) >= 2:
            await msg.answer("🎰 Цього тижня вже зіграв 2 рази.\nПоверни наступного тижня! 🍀")
            return
        result = random.choices([0, 1, 2, 3], weights=[40, 30, 20, 10])[0]
        plays.append(wk)
        luck_log[uid] = plays
        plays_left = 2 - len([d for d in plays if d == wk])
        slots = ["🍋", "🍒", "🍇", "⭐️", "🔔", "💎"]
    ...
    if uid in emoji_game:
        ...
        riddle = game["riddle"]
        game["attempts"] += 1
        answers = riddle.get("answers", [riddle.get("answer", "")])
        if text.lower().strip() in [a.lower() for a in answers]:
            emoji_game.pop(uid, None)
            mark_daily(uid, "emoji_game")
            actual = add_daily_coins(uid, 5)
            correct = answers[0].upper()
            await msg.answer(f"🎉 Правильно! {riddle['emoji']} = {correct}\n+{actual} 🪙")
        elif game["attempts"] >= 3:
            emoji_game.pop(uid, None)
            mark_daily(uid, "emoji_game")
            correct = answers[0].upper()
            await msg.answer(f"😅 Не вгадав!\nПравильна відповідь: {correct} {riddle['emoji']}\nСпробуй завтра!")
        else:
    ...
    if text == "🤫 Таємний друг":
        ...
        await msg.answer("🤫 Твій таємний друг чекає на щось приємне!\n\nНапиши йому анонімне повідомлення 💌")
        return
    ...
    if text.startswith("🧺 Створити збір: "):
        ...
        await msg.answer(f"🧺 Збір на {item['emoji']} {item_name} створено!\nПотрібно: {item['price']} 🪙\n\nСкільки вносиш зараз?")
        return
    ...
    while True:
        ...
        if holidays_sheet:
            try:
                ...
                hrows = holidays_sheet.get_all_records()
                for hr in hrows:
                    bday_date = str(hr.get("Дата", "")).strip()
                    if bday_date == today_dm:
                        imya_key = "Ім'я"
                        bday_name = f"{hr.get('Прізвище', '')} {hr.get(imya_key, '')}".strip()
                        birthdays_today.append(bday_name)
            except Exception:
                pass
